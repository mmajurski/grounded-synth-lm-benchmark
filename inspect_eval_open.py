...
def synth_task(lcl_fp):
    ...
    # model = get_model(model="sgl/Llama-4-Maverick-17B-128E-Instruct", base_url="https://pn131285.nist.gov:8443/v1", config=config)
    model = get_model(model="openai/gpt-4.1-nano", base_url="https://api.openai.com/v1", config=config)
    ...
    return Task(
        ...
        scorer=model_graded_qa(model=model),
    )
